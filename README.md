...
| projectMappings.root.project_selector | A regexp that matches against CloudTruth project names, to limit the projects acted against.  This is a substring match, so use regex anchors to force a full string match, e.g. `^myproject$`.  Supplies any named matches for template evaluation | string | "" | no |
...
For the `project_selector`, which is defined under the *Usage* section, you can either use a regex (i.e - `"^k8s-"`) which matches any CloudTruth project that starts with `k8s-`, or a specific CloudTruth project name. For example, if you have a CloudTruth project called `kubernetes_secrets`, you'll want the `project_selector` value to be `"^kubernetes_secrets$"`. Note the use of regex anchors to prevent other substring matches like `"not_kubernetes_secrets"`.
